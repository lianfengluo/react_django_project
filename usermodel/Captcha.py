...
def CreatePPCaptcha(size=(180, 50),
  ...
  def create_lines():
    ...
    for i in range(line_num):
      ...
      draw.line([begin, end], fill=(random.randint(50, 156), random.randint(
          100, 156), random.randint(100, 156)), width=random.randint(3, 4))
    ...
    for w in range(width):
      for h in range(height):
        ...
        if tmp < point_chance:
          ...
          draw.text((w, h), "*", font=font, fill=(random.randint(180, 200),
                                                  random.randint(200, 220), random.randint(180, 200)))
